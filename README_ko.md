# 이지메이커 마이크로비트 쉴드 확장 (EZmaker Shield for micro:bit)

**BBC micro:bit용 이지메이커 쉴드 확장 라이브러리 및 블록**

이 MakeCode 확장 기능은 마이크로비트용 [이지메이커 쉴드](https://ezmaker.co.kr/eng/)에 연결된 25종 이상의 이지메이커 센서 및 액추에이터를 제어할 수 있는 간결하고 편리한 블록들을 제공합니다.
...
> **공식 승인 대기 중**: 추후 Microsoft 승인이 완료되면 검색창에 `ezmaker` 키워드만 검색하여 공식 지원 리스트에서 바로 설치가 가능해집니다.
